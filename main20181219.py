...
t_str = "{:04}{:02}{:02}T{:02}{:02}{:02}".format(
...
while True:
    ...
    if thisTime.tm_sec != lastRead.tm_sec:  # can be less than when minute ticks over
        lastRead = thisTime  # reset the timer
        for s in sensorIV_array:
            read_array = s.read()
            if mpp:
                i = sensorIV_array.index(s)
                display.displayText("{:.1f} {:.1f} {:.1f}".format(*read_array), i)
                
        
        lipo_voltage = (lipo_volt_pin.value * 3.3) / 65536 * 2
        
        if not mpp:
            display.displayText("{:04}{:02}{:02}T{:02}{:02}{:02}".format(
				thisTime.tm_year, thisTime.tm_mon, thisTime.tm_mday, thisTime.tm_hour, thisTime.tm_min, thisTime.tm_sec), 0)
            display.displayText("{:.1f} {:.1f} {:.1f}".format(sensorIV_0.I, sensorIV_1.I, sensorIV_2.I), 1)
            display.displayText("b:{:.1f}V m:{}b".format(lipo_voltage, gc.mem_free()), 2)
        
    if thisTime.tm_min != lastStore.tm_min:  # every minute take the mean and store the reading
        lastStore = thisTime
        store_str = "{:04}{:02}{:02}T{:02}{:02}{:02}".format(
            thisTime.tm_year, thisTime.tm_mon, thisTime.tm_mday, thisTime.tm_hour, thisTime.tm_min, thisTime.tm_sec)
        ...
        lipo_voltage = (lipo_volt_pin.value * 3.3) / 65536 * 2
        store_str = store_str + ", {:.1f}, {}\n".format(lipo_voltage, gc.mem_free())
